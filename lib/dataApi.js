...
module.exports = {
	getStats: (isin, fee) => {
		const fullUrl = HOST + '/' + ENDPOINT.STATS + '?' + qs.stringify({ isin, fee });
	...
	getComparison: (isin, indexRatio, fee) => {
		const fullUrl = HOST + '/' + ENDPOINT.COMPARISON + '?' + qs.stringify({ isin, indexRatio, fee });
